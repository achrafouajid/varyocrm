...
@Component({
  ...
  template: `
    <div class="flex gap-6">
      ...
      <div class="fixed top-0 left-0 right-0 z-50 glass-strong px-6 lg:px-8 py-4 hidden">
        <!-- Intentionally hidden - global header is in app.ts -->
      </div>

      <!-- Left Sidebar Navigation -->
      <aside class="w-44 shrink-0 hidden lg:block">
        <nav class="space-y-1.5 sticky top-6">
          <button 
            (click)="activeTab.set('Lead')" 
            [class]="activeTab() === 'Lead' ? 'glass-strong text-indigo-700 font-semibold' : 'glass-button text-slate-500 hover:text-indigo-600'"
            class="w-full text-left px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center gap-2.5 border-0">
            <mat-icon class="text-[18px] w-[18px] h-[18px]">filter_alt</mat-icon>
            Leads
            <span class="ml-auto text-xs glass-chip text-slate-500 px-1.5 py-0.5 rounded-full">{{ state.leadsData().length }}</span>
          </button>
          <button 
            ...
            Customers
            <span class="ml-auto text-xs glass-chip text-slate-500 px-1.5 py-0.5 rounded-full">{{ state.customers().length }}</span>
          </button>
          <button 
            (click)="activeTab.set('Prospect')" 
            [class]="activeTab() === 'Prospect' ? 'glass-strong text-indigo-700 font-semibold' : 'glass-button text-slate-500 hover:text-indigo-600'"
            class="w-full text-left px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center gap-2.5 border-0">
            <mat-icon class="text-[18px] w-[18px] h-[18px]">person_search</mat-icon>
            Prospects
            <span class="ml-auto text-xs glass-chip text-slate-500 px-1.5 py-0.5 rounded-full">{{ state.prospects().length }}</span>
          </button>
          <button 
            ...
            Vendors
            <span class="ml-auto text-xs glass-chip text-slate-500 px-1.5 py-0.5 rounded-full">{{ state.vendors().length }}</span>
          </button>
        </nav>
      </aside>
      ...
      <div class="flex-1 min-w-0 space-y-6">
        ...
        <div class="flex justify-end">
          @if (activeTab() === 'Lead') {
            ...
            <button (click)="openCreateModal()" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 shadow-lg shadow-indigo-200">
        ...
        <div class="lg:hidden">
          <nav class="flex gap-2 overflow-x-auto pb-1">
            ...
            (click)="activeTab.set('Lead'); state.breadcrumbLabel.set('Leads')" 
              [class]="activeTab() === 'Lead' ? 'glass-strong text-indigo-700 font-semibold' : 'glass-button text-slate-500'"
              class="whitespace-nowrap px-3.5 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 border-0">
              <mat-icon class="text-[18px] w-[18px] h-[18px]">filter_alt</mat-icon>
              Leads
              <span class="glass-chip text-slate-500 px-1.5 py-0.5 rounded-full text-xs">{{ state.leadsData().length }}</span>
            </button>
            <button 
            (click)="activeTab.set('Customer'); state.breadcrumbLabel.set('Customers')" 
              [class]="activeTab() === 'Customer' ? 'glass-strong text-indigo-700 font-semibold' : 'glass-button text-slate-500'"
              class="whitespace-nowrap px-3.5 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 border-0">
              <mat-icon class="text-[18px] w-[18px] h-[18px]">people</mat-icon>
              Customers
              <span class="glass-chip text-slate-500 px-1.5 py-0.5 rounded-full text-xs">{{ state.customers().length }}</span>
            </button>
            <button 
            (click)="activeTab.set('Prospect'); state.breadcrumbLabel.set('Prospects')" 
              [class]="activeTab() === 'Prospect' ? 'glass-strong text-indigo-700 font-semibold' : 'glass-button text-slate-500'"
              class="whitespace-nowrap px-3.5 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 border-0">
              <mat-icon class="text-[18px] w-[18px] h-[18px]">person_search</mat-icon>
              Prospects
              <span class="glass-chip text-slate-500 px-1.5 py-0.5 rounded-full text-xs">{{ state.prospects().length }}</span>
            </button>
            <button 
            (click)="activeTab.set('Vendor'); state.breadcrumbLabel.set('Vendors')" 
              [class]="activeTab() === 'Vendor' ? 'glass-strong text-indigo-700 font-semibold' : 'glass-button text-slate-500'"
              class="whitespace-nowrap px-3.5 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 border-0">
              <mat-icon class="text-[18px] w-[18px] h-[18px]">store</mat-icon>
              Vendors
              <span class="glass-chip text-slate-500 px-1.5 py-0.5 rounded-full text-xs">{{ state.vendors().length }}</span>
            </button>
          </nav>
        </div>
        ...
        @if (activeTab() === 'Lead') {
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div class="glass-card rounded-2xl p-4 lg:p-5 flex items-center justify-between">
                <div class="space-y-1">
                  ...
                  <div class="text-lg sm:text-xl lg:text-2xl font-bold text-slate-900 truncate">{{ totalLeadsCount() }}</div>
                </div>
              ...
              <div class="glass-card rounded-2xl p-4 lg:p-5 flex items-center justify-between">
                <div class="space-y-1">
                  ...
                  <div class="text-lg sm:text-xl lg:text-2xl font-bold text-slate-900 truncate">{{ qualifiedLeadsCount() }}</div>
                </div>
              ...
              <div class="glass-card rounded-2xl p-4 lg:p-5 flex items-center justify-between">
                <div class="space-y-1">
                  ...
                  <div class="text-lg sm:text-xl lg:text-2xl font-bold text-slate-900 truncate">{{ avgLeadScore() }}%</div>
                </div>
          ...
          @if (selectedLead(); as lead) {
            <div class="fixed inset-0 z-50 overflow-hidden" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
              <div class="absolute inset-0 overflow-hidden">
                <div (click)="closeDetails()" class="absolute inset-0 bg-slate-900/30 backdrop-blur-sm transition-opacity"></div>
                <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
                  <div class="pointer-events-auto w-screen max-w-2xl transform glass-dialog flex flex-col h-full">
                    
                    <!-- Header -->
                    <div class="px-6 py-5 border-b border-white/20 flex items-center justify-between">
                      <div class="flex items-center gap-4">
                        <div class="h-12 w-12 glass-strong text-indigo-700 font-extrabold rounded-xl flex items-center justify-center">
                          {{ getInitials(lead.name) }}
                        </div>
                        <div>
                      ...
                      <div class="flex items-center gap-3">
                        ...
                        <button (click)="closeDetails()" class="w-8 h-8 rounded-lg glass-button flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors">
                    ...
                    <div class="px-6 border-b border-white/20 flex gap-6">
                      <button (click)="activeDetailTab.set('info')" [class]="activeDetailTab() === 'info' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'" class="py-3 border-b-2 text-sm font-semibold transition-all">Info</button>
                      <button (click)="activeDetailTab.set('activities')" [class]="activeDetailTab() === 'activities' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'" class="py-3 border-b-2 text-sm font-semibold transition-all">Activities & Notes</button>
                      <button (click)="activeDetailTab.set('attachments')" [class]="activeDetailTab() === 'attachments' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'" class="py-3 border-b-2 text-sm font-semibold transition-all">Attachments</button>
                      <button (click)="activeDetailTab.set('history')" [class]="activeDetailTab() === 'history' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'" class="py-3 border-b-2 text-sm font-semibold transition-all">Status History</button>
                    </div>
                        ...
                        <div class="space-y-6">
                          <div class="glass rounded-xl p-4 space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm">
                              ...
                              <div><div class="text-[10px] uppercase font-semibold text-slate-400">Email</div><a href="mailto:{{ lead.contacts?.[0]?.email }}" class="font-semibold text-indigo-600 hover:underline mt-0.5 block">{{ lead.contacts?.[0]?.email || '—' }}</a></div>
                              ...
                              @if (lead.contacts?.[0]?.website; as web) {
                                <div><div class="text-[10px] uppercase font-semibold text-slate-400">Website</div><a href="http://{{web}}" target="_blank" class="font-semibold text-indigo-600 hover:underline mt-0.5 block">{{ web }}</a></div>
                              }
                              @if (lead.contacts?.[0]?.linkedin; as li) {
                                <div><div class="text-[10px] uppercase font-semibold text-slate-400">LinkedIn</div><a href="http://{{li}}" target="_blank" class="font-semibold text-indigo-600 hover:underline mt-0.5 block">{{ li }}</a></div>
                              }
                            </div>
                          </div>
                          <div class="glass rounded-xl p-4 space-y-3">
                      ...
                      @if (activeDetailTab() === 'activities') {
                        <div class="space-y-6">
                          ...
                          <div class="space-y-2">
                            ...
                            <div class="glass rounded-xl p-4 text-sm text-slate-700 leading-relaxed whitespace-pre-line">
                          ...
                          <div class="glass-card rounded-xl p-4 space-y-3">
                            ...
                            <div class="flex justify-end pt-2">
                              <button (click)="submitActivity(lead.id)" class="bg-slate-800/80 hover:bg-slate-800 text-white px-3 py-1.5 rounded-lg text-xs font-semibold backdrop-blur-sm">Log Activity</button>
                            </div>
                          </div>
                          <div class="space-y-4">
                        ...
                        <div class="space-y-6">
                          <div class="glass-card rounded-xl p-4 space-y-3">
                            ...
                            <div class="flex gap-3">
                              ...
                              <button (click)="submitAttachment(lead.id)" class="bg-indigo-600/80 hover:bg-indigo-700 text-white px-3 py-2 rounded-lg text-xs font-semibold backdrop-blur-sm">Upload File</button>
                            </div>
                          </div>
                          <div class="space-y-3">
          ...
          @if (addLeadModalOpen()) {
            <div class="fixed inset-0 z-50 bg-slate-900/30 backdrop-blur-sm flex items-center justify-center p-4">
              <div class="glass-dialog rounded-2xl max-w-lg w-full p-6 space-y-4 overflow-y-auto max-h-[90vh]">
                <div class="flex justify-between items-center pb-3 border-b border-white/20">
                  <h3 class="text-lg font-bold text-slate-900">Add New Lead Record</h3>
                  <button (click)="addLeadModalOpen.set(false)" class="w-8 h-8 rounded-lg glass-button flex items-center justify-center text-slate-400 hover:text-slate-600">
                    <mat-icon class="w-5 h-5 text-[20px]! leading-none!">close</mat-icon>
                  </button>
                </div>
                <div class="space-y-4 text-xs font-sans">
                  <div class="space-y-2.5">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      <div><label class="block font-semibold text-slate-500 mb-1">Lead Name*</label><input [(ngModel)]="newLead.name" type="text" placeholder="e.g. John Doe" class="w-full glass-input rounded-lg p-2 outline-none"></div>
                      <div><label class="block font-semibold text-slate-500 mb-1">Company Name*</label><input [(ngModel)]="newLead.companyName" type="text" placeholder="e.g. Acmo Group" class="w-full glass-input rounded-lg p-2 outline-none"></div>
                      <div><label class="block font-semibold text-slate-500 mb-1">Email</label><input [(ngModel)]="newLead.email" type="email" placeholder="e.g. email@acmo.com" class="w-full glass-input rounded-lg p-2 outline-none"></div>
                      <div><label class="block font-semibold text-slate-500 mb-1">Phone</label><input [(ngModel)]="newLead.phone" type="text" placeholder="e.g. +212-6..." class="w-full glass-input rounded-lg p-2 outline-none"></div>
                    </div>
                  </div>
                  <div class="space-y-2.5">
                    <h4 class="font-bold text-indigo-700 uppercase tracking-wider text-[10px]">2. Company Information</h4>
                    <div class="grid grid-cols-2 gap-3">
                      <div><label class="block font-semibold text-slate-500 mb-1">Industry</label><input [(ngModel)]="newLead.industry" type="text" placeholder="e.g. Healthcare" class="w-full glass-input rounded-lg p-2 outline-none"></div>
                      <div><label class="block font-semibold text-slate-500 mb-1">Company Size</label><input [(ngModel)]="newLead.companySize" type="text" placeholder="e.g. 200 employees" class="w-full glass-input rounded-lg p-2 outline-none"></div>
                      <div><label class="block font-semibold text-slate-500 mb-1">City</label><input [(ngModel)]="newLead.city" type="text" placeholder="Casablanca" class="w-full glass-input rounded-lg p-2 outline-none"></div>
                      <div><label class="block font-semibold text-slate-500 mb-1">Country</label><input [(ngModel)]="newLead.country" type="text" placeholder="Morocco" class="w-full glass-input rounded-lg p-2 outline-none"></div>
                    </div>
                  </div>
                  <div class="space-y-2.5">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                      <div><label class="block font-semibold text-slate-500 mb-1">Status</label>
                        <select [(ngModel)]="newLead.status" class="w-full glass-input rounded-lg p-2 outline-none bg-transparent">
                      ...
                      <div><label class="block font-semibold text-slate-500 mb-1">Priority</label>
                        <select [(ngModel)]="newLead.priority" class="w-full glass-input rounded-lg p-2 outline-none bg-transparent">
                          <option value="Low">Low</option><option value="Medium">Medium</option><option value="High">High</option>
                        </select></div>
                      <div><label class="block font-semibold text-slate-500 mb-1">Temperature</label>
                        <select [(ngModel)]="newLead.temperature" class="w-full glass-input rounded-lg p-2 outline-none bg-transparent">
                  ...
                  <div><label class="block font-semibold text-slate-500 mb-1">Notes</label>
                    <textarea [(ngModel)]="newLead.notes" rows="3" placeholder="Evaluate legacy systems, downtime concerns, etc." class="w-full glass-input rounded-lg p-2 outline-none"></textarea></div>
                </div>
                <div class="flex justify-end gap-2 pt-3 border-t border-white/20 text-xs">
                  <button (click)="addLeadModalOpen.set(false)" class="px-4 py-2 glass-button rounded-lg text-slate-600 font-semibold">Cancel</button>
                  <button (click)="saveLead()" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg shadow-lg shadow-indigo-200">Save Lead Record</button>
                </div>
              </div>
            </div>
          }
              ...
              <div class="glass-card rounded-2xl p-6 flex flex-col justify-between">
                <div>
                  <div class="flex items-start justify-between mb-4">
                    <div class="h-12 w-12 rounded-full glass-strong flex items-center justify-center text-lg font-bold text-slate-600">
                      {{partner.name.substring(0,2).toUpperCase()}}
                    </div>
                    <span class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full glass-chip"
                      [class]="partner.type === 'Lead' ? 'text-purple-700' : (partner.type === 'Customer' ? 'text-emerald-700' : (partner.type === 'Prospect' ? 'text-blue-700' : 'text-amber-700'))">
                      {{partner.type}}
                    </span>
                  </div>
                  <h3 class="text-lg font-bold text-slate-900 truncate mb-1">{{partner.name}}</h3>
                  <span class="inline-flex items-center text-xs glass-chip text-slate-600 px-2 py-0.5 rounded mb-4">
                  ...
                  @if(partner.type === 'Lead') {
                    <div class="flex gap-2 flex-wrap mb-4">
                      @if (partner.score) {
                        <span class="px-2 py-0.5 text-[10px] font-bold rounded uppercase tracking-wider glass-chip" [class]="partner.score > 70 ? 'text-emerald-700' : 'text-amber-700'">
                          Score: {{partner.score}}
                        </span>
                      }
                      @if (partner.source) {
                        <span class="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider glass-chip text-indigo-600">
                          {{partner.source}}
                        </span>
                      }
                      @if (partner.assignedTo) {
                        <span class="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider glass-chip text-slate-600 flex items-center gap-1">
                  ...
                  @if(partner.comments) {
                    <div class="mt-4 pt-4 border-t border-white/30 text-xs text-slate-500">
                ...
                <div class="mt-6 pt-4 border-t border-white/30 flex gap-2">
                  @if(partner.type === 'Lead') {
                    <button (click)="state.convertLeadToProspect(partner.id)" class="w-full glass-button rounded-xl px-3 py-2 text-sm font-semibold text-indigo-600 transition-all flex items-center justify-center">
                      <mat-icon class="mr-2 text-[16px] w-4 h-4">arrow_forward</mat-icon>
                      Convert to Prospect
                    </button>
                  }
                  @if(partner.type === 'Prospect') {
                    <button (click)="openConvertModal(partner)" class="w-full glass-button rounded-xl px-3 py-2 text-sm font-semibold text-indigo-600 transition-all flex items-center justify-center">
                      <mat-icon class="mr-2 text-[16px] w-4 h-4">published_with_changes</mat-icon>
                      Convert to Customer
                    </button>
                  }
                  @if(partner.type === 'Customer') {
                    <button (click)="openCustomerCard(partner.id)" class="w-full glass-button rounded-xl px-3 py-2 text-sm font-semibold text-indigo-600 transition-all flex items-center justify-center">
            ...
            <div class="fixed inset-0 z-50 bg-slate-900/30 backdrop-blur-sm flex items-center justify-center p-4">
              <div class="glass-dialog rounded-2xl max-w-md w-full p-6 space-y-6">
                <div class="flex justify-between items-center pb-2 border-b border-white/20">
                  <h3 class="text-lg font-bold text-slate-900">Add New {{newPartner.type}}</h3>
                  <button (click)="showCreateModal.set(false)" class="w-8 h-8 rounded-lg glass-button flex items-center justify-center text-slate-400 hover:text-slate-600">
                ...
                <form (ngSubmit)="savePartner()" class="space-y-4">
                  <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Partner Type</label>
                    <select [(ngModel)]="newPartner.type" name="type" class="w-full glass-input rounded-xl p-2.5 text-sm outline-none bg-transparent">
                  ...
                  <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Company / Contact Name</label>
                    <input [(ngModel)]="newPartner.name" name="name" type="text" placeholder="e.g. Casablanca Technologies" required class="w-full glass-input rounded-xl p-2.5 text-sm outline-none">
                  </div>

                  <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Email</label>
                    <input [(ngModel)]="newPartner.email" name="email" type="email" placeholder="e.g. contact@domain.ma" class="w-full glass-input rounded-xl p-2.5 text-sm outline-none">
                  </div>

                  <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Phone</label>
                    <input [(ngModel)]="newPartner.phone" name="phone" type="text" placeholder="e.g. +212-522-XXXXXX" class="w-full glass-input rounded-xl p-2.5 text-sm outline-none">
                  </div>

                  <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">City</label>
                    <select [(ngModel)]="newPartner.city" name="city" class="w-full glass-input rounded-xl p-2.5 text-sm outline-none bg-transparent">
                  ...
                  @if (newPartner.type === 'Customer') {
                    <div>
                      <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">ICE (15 digits) *</label>
                      <input [(ngModel)]="newPartner.ICE" name="ICE" type="text" maxlength="15" placeholder="e.g. 123456789012345" required class="w-full glass-input rounded-xl p-2.5 text-sm outline-none font-mono">
                    </div>
                    <div>
                      <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Identifiant Fiscal (IF) *</label>
                      <input [(ngModel)]="newPartner.IF" name="IF" type="text" placeholder="e.g. 12345678" required class="w-full glass-input rounded-xl p-2.5 text-sm outline-none font-mono">
                    </div>
                    <div>
                      <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Registre de Commerce (RC) *</label>
                      <input [(ngModel)]="newPartner.RC" name="RC" type="text" placeholder="e.g. 123456" required class="w-full glass-input rounded-xl p-2.5 text-sm outline-none font-mono">
                    </div>
                  }

                  @if (newPartner.type === 'Lead') {
                    <div>
                      <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Lead Score (0-100)</label>
                      <input [(ngModel)]="newPartner.score" name="score" type="number" min="0" max="100" placeholder="e.g. 85" class="w-full glass-input rounded-xl p-2.5 text-sm outline-none">
                    </div>
                    <div>
                      <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Lead Source</label>
                      <select [(ngModel)]="newPartner.source" name="source" class="w-full glass-input rounded-xl p-2.5 text-sm outline-none bg-transparent">
                    ...
                    <div>
                      <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Assigned Salesperson</label>
                      <select [(ngModel)]="newPartner.assignedTo" name="assignedTo" class="w-full glass-input rounded-xl p-2.5 text-sm outline-none bg-transparent">
                        <option value="">-- Unassigned --</option>
                        @for (user of state.users(); track user.name) {
                  ...
                  <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Comments / Notes</label>
                    <textarea [(ngModel)]="newPartner.comments" name="comments" rows="3" placeholder="Additional details..." class="w-full glass-input rounded-xl p-2.5 text-sm outline-none"></textarea>
                  </div>

                  <div class="flex justify-end gap-2 pt-4 border-t border-white/20">
                    <button type="button" (click)="showCreateModal.set(false)" class="px-4 py-2 glass-button rounded-xl text-slate-600 text-sm font-semibold">Cancel</button>
                    <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl shadow-lg shadow-indigo-200">Save Partner</button>
                  </div>
                </form>
              </div>
            </div>
